...
nbd = mfd.nbdof()
print("nbd: ", nbd)
F = gf.asm_boundary_source(
    NEUMANN_BOUNDARY, mim, mfu, mfd, np.repeat([[0], [-100], [0]], nbd, 1)
)
print("F.shape: ", F.shape)
print("mfu.nbdof(): ", mfu.nbdof())
print(
    "np.repeat([[0],[-100],[0]],nbd,1).shape:",
    np.repeat([[0], [-100], [0]], nbd, 1).shape,
)

K = gf.asm_linear_elasticity(
    mim, mfu, mfd, np.repeat([Lambda], nbd), np.repeat([Mu], nbd)
)
...
(H, R) = gf.asm_dirichlet(
    DIRICHLET_BOUNDARY, mim, mfu, mfd, mfd.eval("identity(3)"), mfd.eval("[0,0,0]")
)
...
for i in range(DU.shape[2]):
    ...
    E = (d + d.T) * 0.5
    Sigma[:, :, i] = E
    VM[i] = np.sum(E**2) - (1.0 / 3.0) * np.sum(np.diagonal(E)) ** 2
